...
describe('BigInt Conversion Utils', () => {
  ...
  describe('performSafeBigIntConversion', () => {
    it('should perform USD to wBTC conversion', () => {
      const result = performSafeBigIntConversion('100', usdAsset, wbtcAsset, 50000, true);
      expect(result.isValid).toBe(true);
      expect(result.amount).toBe(BigInt(200000)); // 0.002 wBTC
    });

    it('should handle validation errors', () => {
      const result = performSafeBigIntConversion('invalid', usdAsset, wbtcAsset, 50000, true);
      expect(result.isValid).toBe(false);
      expect(result.error).toBe('Please enter a valid number');
    });

    it('should handle empty input', () => {
      const result = performSafeBigIntConversion('', usdAsset, wbtcAsset, 50000, true);
      expect(result.isValid).toBe(false);
